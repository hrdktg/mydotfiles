...
--mytextclock = awful.widget.textclock(" hello %a %d %b  %H:%M")
...
batwidget = lain.widgets.abase({ 
    timeout=10,
    cmd="sh ~/bin/bat.sh",
    settings = function()
	num=tonumber(output)
	if num<0 then
		widget:set_markup(markup("#EA6F81", num*-1 .. "%"))
	else
		baticon:set_image(beautiful.widget_ac)
		widget:set_markup(markup("#7AC82E", num .. "%"))
	end
    end
})
...
volumewidget = lain.widgets.alsa({
	...
	widget:set_markup(markup(volcol, " " .. volume_now.level .. "% "))
...
netwidget = lain.widgets.abase({
    cmd="sh ~/bin/wifi_stat.sh",
    settings = function()
        if output == "0" then
		widget:set_markup(markup("#EA6F81", "Not Connected"))
	else
		widget:set_markup(markup("#7AC82E", output))
	end
    end
})
